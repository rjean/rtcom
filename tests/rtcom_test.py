...
class TestRealTimeCommunication(unittest.TestCase):
    ...
    def test_device_message(self):
        with RealTimeCommunication("test.device") as rtcom:
            ...
            sleep(0.2)
            self.assertIn("test.device", rtcom.devices)
            self.assertEqual(
                rtcom.listen_thread.devices["test.device"].endpoints["heartbeat"].data,
                10,
            )
            ...
            rtcom.broadcast_endpoint(
                "binary_data", bytes([1, 2, 3, 4]), encoding="binary"
            )
            sleep(0.2)
    ...
    def test_build_big_message(self):
        ...
        for message in messages:
            device, endpoint, data, encoding, id, sequence, max_sequence = read_message(
                message
            )
